...
function PaymentsPanel({ api, canCoordinate, users, activeSeasonId }: { api: ApiClient; canCoordinate: boolean; users: User[]; activeSeasonId: string }) {
  ...
  const paymentGroups = useMemo(() => {
    ...
    return [...groupsMap.values()].map((group): PaymentAthleteGroup | null => {
      ...
      const detailPayments: PaymentRecord[] = [];
      chronologicalPayments.forEach((payment) => {
        if (payment === primaryPayment || paymentTimelineKey(payment) < todayKey || detailPayments.length >= 3) return;
        detailPayments.push(payment);
      });
      reversedPayments.forEach((payment) => {
        if (payment === primaryPayment || paymentTimelineKey(payment) >= todayKey || detailPayments.length >= 15) return;
        detailPayments.push(payment);
      });
      return {
        key: group.key,
  ...
  return (
    <section className="card compact payments-panel">
      ...
      {message && <p className="muted">{message}</p>}
      {organizedPayments.length === 0 ? <EmptyState title="Sem mensalidades lançadas" text="Gere o mês ou registre uma cobrança individual para começar a acompanhar a tabela financeira." /> : <div className="championship-wrap payments-table-wrap"><table className="championship-table payments-table payments-group-table"><thead><tr><th>{renderFilterHeader('Nome', 'payments-name', paymentFilters.name, paymentFilterOptions('name'), (value) => setPaymentFilters((current) => ({ ...current, name: value })), () => setPaymentFilters((current) => ({ ...current, name: '' })), 'Pesquisar nomes')}</th><th>{renderFilterHeader('Mês', 'payments-month', paymentFilters.month, paymentFilterOptions('month'), (value) => setPaymentFilters((current) => ({ ...current, month: value })), () => setPaymentFilters((current) => ({ ...current, month: '' })), 'Pesquisar mês')}</th><th>{renderFilterHeader('Vencimento', 'payments-dueDate', paymentFilters.dueDate, paymentFilterOptions('dueDate'), (value) => setPaymentFilters((current) => ({ ...current, dueDate: value })), () => setPaymentFilters((current) => ({ ...current, dueDate: '' })), 'Pesquisar vencimento')}</th><th>{renderFilterHeader('Valor', 'payments-amount', paymentFilters.amount, paymentFilterOptions('amount'), (value) => setPaymentFilters((current) => ({ ...current, amount: value })), () => setPaymentFilters((current) => ({ ...current, amount: '' })), 'Pesquisar valor')}</th><th>{renderFilterHeader('Pago', 'payments-paid', paymentFilters.paid, paymentFilterOptions('paid'), (value) => setPaymentFilters((current) => ({ ...current, paid: value })), () => setPaymentFilters((current) => ({ ...current, paid: '' })), 'Pesquisar valor pago')}</th><th>{renderFilterHeader('Pendente', 'payments-balance', paymentFilters.balance, paymentFilterOptions('balance'), (value) => setPaymentFilters((current) => ({ ...current, balance: value })), () => setPaymentFilters((current) => ({ ...current, balance: '' })), 'Pesquisar saldo')}</th><th>{renderFilterHeader('Status', 'payments-status', paymentFilters.status, paymentFilterOptions('status'), (value) => setPaymentFilters((current) => ({ ...current, status: value })), () => setPaymentFilters((current) => ({ ...current, status: '' })), 'Pesquisar status')}</th><th>{renderFilterHeader('Pago em', 'payments-paidAt', paymentFilters.paidAt, paymentFilterOptions('paidAt'), (value) => setPaymentFilters((current) => ({ ...current, paidAt: value })), () => setPaymentFilters((current) => ({ ...current, paidAt: '' })), 'Pesquisar data')}</th><th>{renderFilterHeader('Ponto', 'payments-point', paymentFilters.point, paymentFilterOptions('point'), (value) => setPaymentFilters((current) => ({ ...current, point: value })), () => setPaymentFilters((current) => ({ ...current, point: '' })), 'Pesquisar pontuação')}</th><th>{renderFilterHeader('Observação', 'payments-notes', paymentFilters.notes, paymentFilterOptions('notes'), (value) => setPaymentFilters((current) => ({ ...current, notes: value })), () => setPaymentFilters((current) => ({ ...current, notes: '' })), 'Pesquisar observação')}</th>{canCoordinate && <th>Ação</th>}<th>Histórico</th></tr></thead><tbody>{paymentGroups.length === 0 ? <tr><td colSpan={canCoordinate ? 12 : 11} className="table-empty-cell">Nenhuma mensalidade encontrada com os filtros atuais.</td></tr> : paymentGroups.map((group) => <Fragment key={group.key}><tr className={expandedPaymentGroupKey === group.key ? 'payments-group-row is-open' : 'payments-group-row'}><td className="athlete-cell payments-name-cell"><strong>{group.userName}</strong><small>{group.payments.length} mensalidade(s) registradas{group.matchingPayments.length !== group.payments.length ? ` • ${group.matchingPayments.length} no filtro` : ''}</small></td><td>{paymentMonthLabel(group.primaryPayment.referenceMonth)}</td><td>{group.primaryPayment.dueDate?.slice(0, 10) ?? '-'}</td><td>{money(group.primaryPayment.amountCents)}</td><td>{money(group.primaryPayment.paidAmountCents ?? 0)}</td><td className="payments-balance-cell">{money(group.primaryPayment.balanceCents ?? 0)}</td><td><span className={`status ${paymentStatusTone(group.primaryPayment.status)}`}>{statusLabel(group.primaryPayment.status)}</span></td><td>{group.primaryPayment.paidAt ? group.primaryPayment.paidAt.slice(0, 10) : 'Nao informado'}</td><td>{group.primaryPayment.earnsPoint ? <span className="status open">+1 pt</span> : <span className="payments-muted">Sem ponto</span>}</td><td className="payments-notes-cell">{group.primaryPayment.notes?.trim() ? group.primaryPayment.notes : <span className="payments-muted">-</span>}</td>{canCoordinate && <td className="payments-action-cell"><button type="button" className="ghost small" onClick={() => openRegister(group.primaryPayment)}>{(group.primaryPayment.balanceCents ?? 0) > 0 ? 'Baixar saldo' : 'Editar'}</button></td>}<td className="payments-history-toggle-cell"><button type="button" className="ghost small payments-expand-button" onClick={() => setExpandedPaymentGroupKey((current) => current === group.key ? null : group.key)}>{expandedPaymentGroupKey === group.key ? 'Ocultar' : `Ver mais ${group.detailPayments.length}`}</button></td></tr>{expandedPaymentGroupKey === group.key && <tr className="payments-group-detail-row"><td colSpan={canCoordinate ? 12 : 11}><div className="payments-subtable-wrap"><div className="payments-subtable-head"><strong>Outras mensalidades de {group.userName}</strong><span>Próximas 3 e últimas 12</span></div>{group.detailPayments.length === 0 ? <p className="payments-muted">Não há outras mensalidades para mostrar além da linha principal.</p> : <table className="championship-table payments-table payments-subtable"><thead><tr><th>Mês</th><th>Vencimento</th><th>Valor</th><th>Pago</th><th>Pendente</th><th>Status</th><th>Pago em</th><th>Ponto</th><th>Observação</th>{canCoordinate && <th>Ação</th>}</tr></thead><tbody>{group.detailPayments.map((payment) => <tr key={`${group.key}-${payment.id ?? payment.referenceMonth}-${payment.dueDate ?? 'sem-vencimento'}`}><td>{paymentMonthLabel(payment.referenceMonth)}</td><td>{payment.dueDate?.slice(0, 10) ?? '-'}</td><td>{money(payment.amountCents)}</td><td>{money(payment.paidAmountCents ?? 0)}</td><td className="payments-balance-cell">{money(payment.balanceCents ?? 0)}</td><td><span className={`status ${paymentStatusTone(payment.status)}`}>{statusLabel(payment.status)}</span></td><td>{payment.paidAt ? payment.paidAt.slice(0, 10) : 'Nao informado'}</td><td>{payment.earnsPoint ? <span className="status open">+1 pt</span> : <span className="payments-muted">Sem ponto</span>}</td><td className="payments-notes-cell">{payment.notes?.trim() ? payment.notes : <span className="payments-muted">-</span>}</td>{canCoordinate && <td className="payments-action-cell"><button type="button" className="ghost small" onClick={() => openRegister(payment)}>{(payment.balanceCents ?? 0) > 0 ? 'Baixar saldo' : 'Editar'}</button></td>}</tr>)}</tbody></table>}</div></td></tr>}</Fragment>)}</tbody></table></div>}
