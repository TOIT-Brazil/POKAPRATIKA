...
export function App() {
  ...
  return (
    <main className={`shell ${view === 'temporada' ? 'shell-home' : ''} ${accountMenuOpen ? 'menu-open' : ''}`}>
      ...
      {view === 'temporada' && <div className="home-stack dashboard-main"><div className="dashboard-top-grid"><DashboardMatchesPanel api={api} canCoordinate={canCoordinate} users={users} matches={matches} rankings={rankings} standings={standings} activeSeasonId={activeSeasonId} currentUserId={auth.user.id} onReload={loadData} selectedMatch={selectedMatch} setSelectedMatch={setSelectedMatch} /></div><div className="dashboard-bottom-grid"><DashboardStandingsPanel standings={standings} onOpenProfile={setProfileUserId} /></div></div>}
...
function DashboardMatchesPanel({ api, canCoordinate, users, matches, rankings, standings, activeSeasonId, currentUserId, onReload, selectedMatch, setSelectedMatch }: { api: ApiClient; canCoordinate: boolean; users: User[]; matches: MatchListItem[]; rankings: RankingPayload; standings: Standing[]; activeSeasonId: string; currentUserId: string; onReload: () => Promise<void>; selectedMatch: MatchDetail | null; setSelectedMatch: (match: MatchDetail | null) => void }) {
  ...
  return (
    <section className="card compact matches-report dashboard-next-match-card">
      ...
      {leaderBubbles.length > 0 && <div className="next-match-leader-strip">{leaderBubbles.map((item) => <article className={`next-match-leader-bubble ${item.accent}`} key={item.key}><div className="next-match-leader-stat"><span className="next-match-leader-value">{item.value}</span><strong>{item.label}</strong></div><small className="next-match-leader-name">{item.name}</small></article>)}</div>}
