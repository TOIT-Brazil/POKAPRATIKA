...
function PaymentsPanel({ api, canCoordinate, users, activeSeasonId }: { api: ApiClient; canCoordinate: boolean; users: User[]; activeSeasonId: string }) {
  ...
  const paymentGroups = useMemo(() => {
    ...
    return [...groupsMap.values()].map((group): PaymentAthleteGroup | null => {
      ...
      const upcomingDetailPayments = chronologicalPayments.filter((payment) => payment !== primaryPayment && paymentTimelineKey(payment) >= todayKey).slice(0, 3);
      const previousDetailPayments = reversedPayments.filter((payment) => payment !== primaryPayment && paymentTimelineKey(payment) < todayKey).slice(0, 6);
