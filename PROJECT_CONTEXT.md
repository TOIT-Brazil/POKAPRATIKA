- A súmula operacional agora preserva trocas manuais também antes do início oficial enquanto o autosave consolida a edição, mantém `startsOnBench` salvo ao reconstruir a escalação e preserva o histórico local da rotação automática durante o refresh periódico; além disso, a normalização da escalação passou a redistribuir goleiros naturais entre os times quando uma recomposição deixa os dois no mesmo lado, e o backend passou a revalidar/normalizar o papel operacional salvo antes do `start` e a validar goleiro/linhas apenas entre os titulares em quadra, sem contar o banco.
